...
export function applyActivityTrigger(record, payload) {
  ...
  const hasMeaningfulDistance = distanceMeters > MOVEMENT_THRESHOLD_METERS;
  const hasActivityDelta = hasMeaningfulDistance || stepsDelta > 0 || activeMinutesDelta > 0;

  if (!hasActivityDelta) {
    return {
      updated: false,
      reason: 'no_activity_delta',
  ...
  const reportableDistanceMeters = hasMeaningfulDistance ? distanceMeters : 0;
  const transportImpact = calculateTransportImpact(payload.activityType, reportableDistanceMeters);

  record.steps += stepsDelta;
  record.active_time = round(record.active_time + activeMinutesDelta, 2);
  record.activity_distance = round(record.activity_distance + reportableDistanceMeters / 1000, 3);
  ...
  record.pending_activity.distance_m = round(record.pending_activity.distance_m + reportableDistanceMeters, 2);
  ...
  pushLimited(record.activity_logs, {
    timestamp,
    distance_moved: round(reportableDistanceMeters, 2),
