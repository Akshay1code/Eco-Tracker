...
export async function updateUserProfileSettings(userId, profileData = {}) {
  ...
  const nextName =
    typeof profileData.name === 'string' && profileData.name.trim().length >= 2
      ...
      : existingUser.name;
  const nextNickname =
    typeof profileData.nickname === 'string' && profileData.nickname.trim().length >= 2
      ? profileData.nickname.trim()
      : existingUser.nickname;
  const nextDob =
    typeof profileData.dob === 'string' && profileData.dob.trim()
      ? profileData.dob.trim()
      : existingUser.dob;
  const nextCity =
    typeof profileData.city === 'string' && profileData.city.trim().length >= 2
      ? profileData.city.trim()
      : existingUser.city;
  const nextCountry =
    typeof profileData.country === 'string' && profileData.country.trim().length >= 2
  ...
  if (isFileStoreMode()) {
    await updateStoredUserByEmailKey(normalizeLookupValue(userId), (currentUser) => ({
      ...
      nicknameKey: normalizeLookupValue(nextNickname),
...
export async function deleteUserAccount(userId) {
  const normalizedUserId = normalizeLookupValue(userId);
  ...
  if (isFileStoreMode()) {
    await deleteStoredUserByEmailKey(normalizedUserId);
  } else {
    await getUsersCollection().deleteOne({ emailKey: normalizedUserId });
  }

  return sanitizeUserDocument(existingUser);
}
