...
export async function updateStoredUserByEmailKey(emailKey, updater) {
  ...
  if (
    nextUser.emailKey &&
    store.users.some((user, index) => index !== userIndex && user.emailKey === nextUser.emailKey)
  ) {
    throw createDuplicateError('emailKey');
  }

  if (
    nextUser.nicknameKey &&
    store.users.some((user, index) => index !== userIndex && user.nicknameKey === nextUser.nicknameKey)
  ) {
    throw createDuplicateError('nicknameKey');
  }
...
export async function deleteStoredUserByEmailKey(emailKey) {
  const store = await readStore();
  const userIndex = store.users.findIndex((user) => user.emailKey === emailKey);
  if (userIndex < 0) {
    return null;
