...
function saveDailyStats(stats) {
  try { localStorage.setItem(`eco_daily_${todayKey()}`, JSON.stringify(stats)); } catch { /* ignore storage write errors */ }
}
...
function saveHourlyHistory(h) {
  try { localStorage.setItem(`eco_hourly_${todayKey()}`, JSON.stringify(h)); } catch { /* ignore storage write errors */ }
}
...
function saveStreak(days) {
  try { localStorage.setItem("eco_streak", JSON.stringify({ days, lastDate: todayKey() })); } catch { /* ignore storage write errors */ }
}
...
function useDeviceCarbonTracker(userEmail) {
  const getTransportFactor = (speedKmh) => {
    if (speedKmh < 1) return 0;        // Stationary
    if (speedKmh < 8) return 0.00002;  // Walking
    if (speedKmh < 25) return 0.0001;   // Cycling
    if (speedKmh < 80) return 0.12;     // City Transit
    return 0.21;                         // Highway/Car
  };

  const CARBON_PER_KM = 0.0002;
  const MAX_ACCEPTED_GPS_ACCURACY_M = 100; // Relaxed from 50
  const BASE_MIN_MOVEMENT_KM = 0.002;      // Relaxed from 0.003
  ...
  const stepDistanceCarryRef = useRef(0);
  ...
  const reverseGeocode = useCallback(async (latitude, longitude) => {
    try {
      ...
      if (!response.ok) {
        addressRef.current = "Address service busy";
        return;
      }
      const data = await response.json();
      const resolved = data?.city || data?.locality || data?.principalSubdivision;
      if (resolved) addressRef.current = resolved;
      else addressRef.current = "Address not found";
    } catch {
      addressRef.current = "Network error while geocoding";
    }
  }, [userEmail]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      saveDailyStats({
        ...
        maxSpeed: Math.max(loadDailyStats()?.maxSpeed || 0, speedRef.current),
  ...
  const acquireWakeLock = useCallback(async () => {
    ...
    try {
      ...
      wakeLockRef.current.addEventListener("release", () => {
        setTimeout(() => {
          if (!document.hidden && supported.wakeLock) {
            navigator.wakeLock.request("screen")
              .then((lock) => { wakeLockRef.current = lock; })
              .catch(() => { /* ignore wake lock reacquire failures */ });
          }
        }, 2000);
      });
    } catch { /* ignore wake lock acquisition failures */ }
  }, [supported.wakeLock]);

  const enableMotionTracking = useCallback(async (isUserGesture = false) => {
    if (motionHandlerRef.current) return true;
    if (typeof window === "undefined" || !("DeviceMotionEvent" in window)) return false;

    try {
      if (typeof DeviceMotionEvent !== "undefined" && typeof DeviceMotionEvent.requestPermission === "function") {
        if (!isUserGesture) {
          // Can't request without gesture, just check if we already have it
          // Note: requestPermission() only works on user gesture. 
          // If we are in useEffect, we just skip and wait for user to click "Resume"
          return false;
        }
        ...
        setMotionPermission("granted"); // Non-iOS browsers usually don't need explicit requestPermission
      }
    } catch (e) {
    ...
    const handleMotion = (event) => {
      ...
      // Some browsers expose acceleration values in g-units (~1 at rest) instead of m/s^2 (~9.81 at rest).
      if (magnitude > 0.1 && magnitude < 3.5) magnitude *= 9.81;
      const linearMotion = Math.abs(magnitude - 9.81);
      const now = Date.now();
      // Use movement above gravity baseline instead of raw magnitude for more reliable cross-device step detection.
      if (linearMotion > 1.2 && (now - lastStepTimeRef.current) > 280) {
        stepsRef.current += 1;
  ...
  const startTracking = useCallback((isUserGesture = false) => {
    if (!userEmail || watchIdRef.current) return;
    setIsTracking(true);
    acquireWakeLock();
    enableMotionTracking(isUserGesture);
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (hasGoodAccuracy && !isJitter && !isOutlierJump) {
            ...
            if (currentSpeedKmh > 0.4 && currentSpeedKmh < 12) {
              stepDistanceCarryRef.current += stepKm;
              const stepsFromDistance = Math.floor(stepDistanceCarryRef.current / 0.00075);
              if (stepsFromDistance > 0) {
                stepsRef.current += stepsFromDistance;
                stepDistanceCarryRef.current -= stepsFromDistance * 0.00075;
              }
            }
  ...
  }, [userEmail, acquireWakeLock, enableMotionTracking, maybeReverseGeocode]);

  const stopTracking = useCallback(() => {
    setIsTracking(false);
    stepDistanceCarryRef.current = 0;
    if (motionHandlerRef.current) {
      window.removeEventListener("devicemotion", motionHandlerRef.current);
      motionHandlerRef.current = null;
    }
  ...
  useEffect(() => {
    if (!userEmail || permissionState !== "granted" || isTracking) return;
    const id = setTimeout(() => startTracking(), 0);
    return () => clearTimeout(id);
  }, [userEmail, permissionState, isTracking, startTracking]);

  return {
    carbon, batteryUsed, distance, speed, steps, location, address, screenTime,
    permissionDenied, permissionState, motionPermission, isTracking, supported,
    startTracking, stopTracking, enableMotionTracking,
  };
}
...
function Particles({ trigger }) {
  ...
  const ps = React.useMemo(() => {
    ...
    return Array.from({ length: 32 }, (_, i) => ({
      id: i,
      x: 30 + deterministicRand(trigger * 101 + i * 17) * 40,
      y: 20 + deterministicRand(trigger * 103 + i * 19) * 60,
      dx: (deterministicRand(trigger * 107 + i * 23) - 0.5) * 240,
      dy: -(60 + deterministicRand(trigger * 109 + i * 29) * 180),
      color: ["#22c55e", "#86efac", "#fbbf24", "#60a5fa", "#f0abfc", "#fdba74"][i % 6],
      size: 4 + deterministicRand(trigger * 113 + i * 31) * 7,
    }));
  }, [trigger]);
  return (
...
function PermissionGate({ onGrant, supported }) {
  ...
  const handleRequest = async () => {
    ...
    try {
      // Start tracking immediately from the click gesture so motion permission requests can succeed on iOS.
      onGrant(true);
    ...
    <div style={{ background: "linear-gradient(135deg,rgba(251,146,60,0.06),rgba(234,88,12,0.03))", border: "1px solid rgba(251,146,60,0.18)", borderRadius: 18, padding: "20px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 12 }}>
        {[
          ...
          { icon: "⚡", label: "MAX SPEED", value: `${(stats.maxSpeed || 0).toFixed(1)} km/h`, color: "#06b6d4" },
          { icon: "👣", label: "TOTAL STEPS", value: (stats.steps || 0).toLocaleString(), color: "#22bc9e" },
        ...
        <div className="dashboard-main" style={{ maxWidth: 1140, margin: "0 auto", padding: "0 24px 64px" }}>
          ...
          <div className="dashboard-hero-grid" style={{ display: "grid", gridTemplateColumns: "196px 1fr", gap: 20, marginBottom: 20 }}>
            ...
            <div className="dashboard-stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
              ...
              <StatCard delay={225} icon="👣" label="Steps" value={steps.toLocaleString()} unit="steps" glow="#22bc9e" />
