...
export default function useGoogleFit(userEmail) {
  ...
  const beginGoogleFitConnection = useCallback(
    async (prompt = 'consent', suppressErrors = false) => {
      if (!configured) {
        if (!suppressErrors) {
          setFitError(
      ...
      try {
        ...
        const isExpectedAutoConnectMiss =
          message.includes('popup_closed') ||
          message.includes('access_denied') ||
          message.includes('immediate_failed') ||
          message.includes('interaction_required');

        if (!suppressErrors || !isExpectedAutoConnectMiss) {
  ...
  const connectGoogleFit = useCallback(async () => {
    await beginGoogleFitConnection('consent', false);
  }, [beginGoogleFitConnection]);
